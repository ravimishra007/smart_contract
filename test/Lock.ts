...
const _checkClaim = async (contract: TokenFarm, token: CarRent, user: Signer) => {
  ...
  const claimableReward = await contract.claimableRewards((await user.getAddress()));
  if (claimableReward == 0n) {
    expect(await contract.connect(user).claimRewards()).to.be.revertedWith("No rewards available");
    return
  }
...
const _checkEthClaim = async (contract: TokenFarm, user: Signer) => {
  const beforeUserEthBal = await ethers.provider.getBalance((await user.getAddress()));
  const claimableEthReward = await contract.claimableETHReward((await user.getAddress()));
  if (claimableEthReward == 0n) {
    expect(await contract.connect(user).claimETHReward()).to.be.revertedWith("No rewards available");
    return
  }
  const tx = await (await contract.connect(user).claimETHReward()).wait();
  const afterUserEthBal = await ethers.provider.getBalance((await user.getAddress()));
  expect(afterUserEthBal).to.eq(beforeUserEthBal + claimableEthReward - ((tx?.gasUsed ?? 0n) * (tx?.gasPrice ?? 0n)));
}
...
describe("Should test the lexa contract", () => {
  ...
  it("Should deploy the token and lexa contract and create a pool", async function () {
    ...
    const lexaContract = await lexaFactory.connect(owner).deploy(tokenContract.target, tokenContract.target);
    await lexaContract.waitForDeployment();

    const addPool = await lexaContract.connect(owner).addPool(toWei(1e8), toWei(1e6), 50, toWei(100), 100, 10, true, true);
    await addPool.wait();
  ...
  it("Should add the eth reward", async function () {
    const [owner, user1, user2] = await ethers.getSigners();
    const addEthReward = await lexa.connect(owner).addETHReward({ value: toWei(9) });
  ...
  it("Should claim the eth reward", async function () {
    const [owner, user1, user2, user3] = await ethers.getSigners();
    const claimableEthRewardUser1 = await lexa.claimableETHReward(user1.address);
    console.log('claimableEthRewardUser1: ', fromWei(claimableEthRewardUser1));
    const claimableEthRewardUser2 = await lexa.claimableETHReward(user2.address);
    console.log('claimableEthRewardUser2: ', fromWei(claimableEthRewardUser2));
    const claimableEthRewardUser3 = await lexa.claimableETHReward(user3.address);
